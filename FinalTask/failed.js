...
class BarChart {
  ...
  //      self.trans = a => a[0].map((_, c) => a.map(r => r[c]));

        self.stackGen = d3.stack().keys(["paper", "digital"]);
        self.series = self.stackGen(self.data);

  //      self.series = self.trans(self.stackGen(self.data));
...
//            .attr("fill", function(d) { return self.color(d.key); } )
      ...
      //    .join("rect")
            ...
            .attr("y", function(d) { return self.yscale(d.data.paper); } )
            .attr("width", self.xscale.bandwidth())
            .attr("height", function(d) { return self.yscale(d[0]) - self.yscale(d[1]); } );
      //      .attr("height", function(d) { return self.yscale(d.data.paper); } );
